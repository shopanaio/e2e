...
test.describe('Category gallery connection', () => {
  ...
  test('paginates gallery forward', async ({ api }) => {
    ...
    // Запрашиваем оставшиеся файлы начиная с endCursor
  ...
  test('paginates gallery backward', async ({ api }) => {
    ...
    // Запрашиваем предыдущие два файла до startCursor
  ...
  // Помощник для декодирования Relay-курсов Base64URL → объект
  ...
  test('paginates gallery with arbitrary cursor', async ({ api }) => {
    ...
    // Получаем полный список, берём курсор второго элемента
    ...
    const randomCursor = listEdges[1].cursor; // cursor второго файла

    // Запрашиваем один файл после randomCursor
    const { data } = await api.client.query('client/CategoryGallery', {
  ...
  test('round-trip pagination integrity', async ({ api }) => {
    ...
    while (true) {
      ...
      // Проверяем корректность endCursor → sort_index
